...
public class FloorMaker : MonoBehaviour {
    ...
    public int myCounter = 0;            //	Declare a private integer called myCounter that starts at 0; 		// count how many floor tiles this FloorMaker has instantiated
    public int counterMax;               // set max tiles maker makes before being destroyed    set in inspector
    ...
    void Update () {
        if (myCounter < counterMax)
        {
            ...
            if (randomCheck < 0.25f)
            {
                transform.Rotate(0f, 0f, 90f, Space.World);
            }
            else if (randomCheck >= .25f && randomCheck < 0.5f)
            {
                transform.Rotate(0f, 0f, -90f, Space.World);
            }
            else if (randomCheck >= 0.99f && randomCheck <= 1.0f)
            {
